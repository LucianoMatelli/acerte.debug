...
STATUS_MAP = {
    "A Receber/Recebendo Proposta": "recebendo_propostas",
    "Em Julgamento/Propostas Encerradas": "propostas_encerradas",
    "Encerradas": "encerradas",
    "Todos": "",
}
STATUS_ALIASES = {
    "recebendo_propostas": ["recebendo_propostas", "recebendo_proposta"],
    "propostas_encerradas": ["propostas_encerradas", "em_julgamento"],
    "encerradas": ["encerradas", "encerrado"],
    "": [""],
}
...
def _items_from_json(js) -> List[Dict]:
    if isinstance(js, dict):
        for k in [
            "items", "results", "conteudo", "licitacoes", "data",
...
# ==========================
# Coleta PNCP (corrigida)
# ==========================
def consultar_pncp_por_municipio(
    municipio_id: str,
    status_value: str = "recebendo_propostas",
    tam_pagina: int = TAM_PAGINA_FIXO,
    delay_s: float = 0.05,
) -> List[Dict]:
    out: List[Dict] = []
    pagina = 1
    status_candidates = STATUS_ALIASES.get(status_value, [status_value])

    while True:
        itens_pagina: List[Dict] = []
        last_err = None

        for api_url in BASE_APIS:
            for st_candidate in status_candidates:
                params = {
                ...
                if st_candidate:
                    params["status"] = st_candidate

                try:
                    r = requests.get(api_url, params=params, headers=HEADERS, timeout=30)
                    r.raise_for_status()
                    js = r.json()
                    itens_pagina = _items_from_json(js)
                    if itens_pagina:
                        break
                except Exception as e:
                    last_err = e
        ...
        if not itens_pagina:
            if last_err and pagina == 1:
                raise RuntimeError(f"Falha ao consultar PNCP (município {municipio_id}): {last_err}")
            break

        out.extend(itens_pagina)

        if len(itens_pagina) < tam_pagina:
            break
